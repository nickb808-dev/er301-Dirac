...
local controlOrder = {
  "trig", "hold", "semishift", "playhead", "grainlen", "grains", "rate",
  "posjtr", "spread", "detune",
  "psprd", "texture", "compress", "revprob", "level", "mix", "feedback", "binaural",
}
...
function Dirac:onLoadViews(objects, branches)
  ...
  gb("rate",     "dens",  "Density; 0 = trig only", app.LinearDialMap(0, 16), 3.0, app.LinearDialMap(-16, 16))
  gb("posjtr",   "posJtr","Position jitter", Encoder.getMap("[0,1]"), 0.0)
  gb("spread",   "sprd",  "Stereo spread", Encoder.getMap("[0,1]"), 0.5)
  gb("detune",   "detun", "Stereo detune (st)", app.LinearDialMap(0, 0.5), 0.0, app.LinearDialMap(-0.5, 0.5))
  gb("level",    "level", "Output level", Encoder.getMap("[0,1]"), 0.7)
  gb("mix",      "mix",   "Wet/dry (live)", Encoder.getMap("[0,1]"), 1.0)
  gb("feedback", "fdbk",  "Feedback regen (live)", Encoder.getMap("[0,1]"), 0.0)
  gb("compress", "comp",  "Grain leveling", Encoder.getMap("[0,1]"), 0.0)
  gb("binaural", "bin",   "Binaural 3D (ITD+shadow)", Encoder.getMap("[0,1]"), 0.0)
  ...
  gb("texture",  "textr", "Shape: perc–round–flat", Encoder.getMap("[0,1]"), 0.5)
  ...
  controls.trig = Gate { button = "trig", description = "Trigger — one grain per rising edge",
                         branch = branches.fire, comparator = objects.fire }

  controls.hold = Gate { button = "hold", description = "Hold (latched) — loop grains as a frozen cloud",
